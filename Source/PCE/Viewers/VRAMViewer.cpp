...
void FVRAMViewer::ClearUsage()
{
	pPCEEmu->GetVRAMAnalysisState()->ClearUsage();
}
...
void FVRAMViewer::DrawPhysicalMemoryOverview()
{
	...
	{
		const int lastWriteFrame = pVRAMState->GetLastVRAMWriteFrame();
		const int framesSince = lastWriteFrame == -1 ? 255 : curFrame - lastWriteFrame;
		...
		ImGui::SameLine();
		const FAddressRef lastWriter = pVRAMState->GetLastVRAMWriter();
