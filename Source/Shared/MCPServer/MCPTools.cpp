	...
	nlohmann::json Execute(FEmuBase* pEmu, const nlohmann::json& arguments) override
	{
		...
		for (const auto& funcIt : codeAnalysis.pFunctions->GetFunctions())
		{
			...
			if(pLabel != nullptr)
			{
				...
				funcJson["address"] = func.StartAddress.GetAddress();
		...
		if (pFuncInfo)
		{
			...
			if (pFuncInfo->CallPoints.empty() == false)
			{
				...
				for (const auto& callPoint : pFuncInfo->CallPoints)
				{
					...
					callPointJson["address"] = callPoint.CallAddr.GetAddress();
			...
			if (pFuncInfo->ExitPoints.empty() == false)
			{
				...
				for (const auto& exitPoint : pFuncInfo->ExitPoints)
				{
					...
					exitPointJson["address"] = exitPoint.GetAddress();
		...
		nlohmann::json Execute(FEmuBase* pEmu, const nlohmann::json& arguments) override
		{
			...
			if(pCodeInfo)
			{
				...
				if (pCodeInfo->Text.empty())
				{
					...
					case ECPUType::HuC6280: HuC6280DisassembleCodeInfoItem(address, codeAnalysis, pCodeInfo); break;
					default: break;
					}
				}
				...
				{
					reads.push_back(read.GetAddress());
				}
				...
				{
					writes.push_back(write.GetAddress());
				}
...
public:
	...
	nlohmann::json Execute(FEmuBase* pEmu, const nlohmann::json& arguments) override
	{
		...
		if (pDataInfo)
		{
			...
			result["last_writer"] = pDataInfo->LastWriter.GetAddress();
...
public:
	...
	{
		FCodeAnalysisState& codeAnalysis = pEmu->GetCodeAnalysis();
		const auto& trace = codeAnalysis.Debugger.GetFrameTrace();
		...
		for (const FAddressRef& startAddr : order)
		{
			...
			snprintf(addrStr, sizeof(addrStr), "$%04X", startAddr.GetAddress());
...
public:
	...
	nlohmann::json Execute(FEmuBase* pEmu, const nlohmann::json& arguments) override
	{
		...
		auto makeEntry = [&](FAddressRef ref) -> nlohmann::json
		{
			nlohmann::json entry;
			char addrStr[8];
			snprintf(addrStr, sizeof(addrStr), "$%04X", ref.GetAddress());
			entry["address"] = addrStr;
		...
		nlohmann::json result;
		char targetStr[8];
		snprintf(targetStr, sizeof(targetStr), "$%04X", addrRef.GetAddress());
		...
		for (const auto& funcIt : codeAnalysis.pFunctions->GetFunctions())
		{
			...
			for (const auto& cp : func.CallPoints)
			{
				if (cp.FunctionAddr == addrRef)
				{
					nlohmann::json entry;
					char callStr[8];
					snprintf(callStr, sizeof(callStr), "$%04X", cp.CallAddr.GetAddress());
					...
					{
						char funcAddrStr[8];
						snprintf(funcAddrStr, sizeof(funcAddrStr), "$%04X", func.StartAddress.GetAddress());
...
public:
	...
	nlohmann::json Execute(FEmuBase* pEmu, const nlohmann::json& arguments) override
	{
		...
		const FFunctionInfo* pTarget = codeAnalysis.FindFunctionByName(functionName.c_str());
		if (!pTarget)
			return { {"error", "Function not found: " + functionName} };
		...
		char targetStr[8];
		snprintf(targetStr, sizeof(targetStr), "$%04X", targetAddr.GetAddress());
		...
		for (const auto& ref : pTargetLabel->References.GetReferences())
		{
			...
			char refStr[8];
			snprintf(refStr, sizeof(refStr), "$%04X", ref.GetAddress());
			...
			if (it == callerMap.end())
			{
				...
				{
					char funcAddrStr[8];
					snprintf(funcAddrStr, sizeof(funcAddrStr), "$%04X", pCallerFunc->StartAddress.GetAddress());
					entry["caller"] = funcAddrStr;
				}
				char funcStartStr[8];
				snprintf(funcStartStr, sizeof(funcStartStr), "$%04X", pCallerFunc->StartAddress.GetAddress());
