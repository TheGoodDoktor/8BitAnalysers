...
void ApplySuggestion(FEmuBase* pEmu, const FMCPSuggestion& suggestion)
{
	...
	{
		FFunctionInfo* pFuncInfo = codeAnalysis.FindFunctionByName(suggestion.TargetName.c_str());
		if (pFuncInfo)
		{
...
void FMCPSuggestionQueue::Save(const std::string& filePath) const
{
	...
	for (const auto& s : Suggestions)
	{
		...
		item["bankId"]     = s.AddressRef.GetBankId();
		item["address"]    = s.AddressRef.GetAddress();
