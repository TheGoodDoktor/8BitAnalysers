...
void Do8BitLoadBlock(uint8_t y, uint8_t z, uint8_t prefix, uint32_t *r, uint32_t& regFlags)
{
	if (y == 6) 
	{
		if (z == 6) 
		...
		else 
		{
			...
			if (prefix && ((z == 4) || (z == 5))) 
			{
				...
				snprintf(g_TTZ80DescBuf, kTTZ80DescLen, "Load %s from %s.", GetRegName(r[6]).c_str(), GetRegName(g_TTZ80Reg[z]).c_str());
				regFlags = g_TTZ80Reg[z] | r[6];
			}
			else 
			{
				snprintf(g_TTZ80DescBuf, kTTZ80DescLen, "Load %s from %s.", GetRegName(r[6]).c_str(), GetRegName(r[z]).c_str());
				regFlags = r[z] | r[6];
			}
		}
	}
	else if (z == 6) 
	{
		...
		if (prefix && ((y == 4) || (y == 5))) 
		{
			...
			snprintf(g_TTZ80DescBuf, kTTZ80DescLen, "Load %s from %s.", GetRegName(g_TTZ80Reg[y]).c_str(), GetRegName(r[6]).c_str() );
			regFlags = g_TTZ80Reg[y] | r[6];
		}
		else 
		{
			snprintf(g_TTZ80DescBuf, kTTZ80DescLen, "Load %s from %s.", GetRegName(r[y]).c_str(), GetRegName(r[6]).c_str());
			regFlags = r[y] | r[6];
		}
	}
	else 
	{
		...
		regFlags = g_TTZ80Reg[y] | g_TTZ80Reg[z];
	}
}

void Do8BitALUBlock(uint8_t y, uint8_t z, uint32_t *r, uint32_t& regFlags)
{
	const std::string regName = GetRegName(r[z]);
	...
	regFlags |= r[z] | Z80Reg::A;
}

void DoCBPrefix(uint8_t prefix, uint32_t *r, uint16_t pc, ICPUInterface* CPUIF, uint32_t& regFlags)
{
    int8_t d = 0;
	uint8_t op = 0;
...
void DoEDPrefix(uint8_t prefix, uint32_t *r, uint32_t *rp, uint16_t pc, ICPUInterface* CPUIF, uint32_t& regFlags)
{
	z80_t* pCPU = (z80_t *)CPUIF->GetCPUEmulator();
    ...
    else if (x == 2) 
	{
        if ((y >= 4) && (z <= 3)) 
		{
			...
			if (z == 0) /* First column. LDI, LDD, LDIR, LDDR*/
			{
				...
				regFlags = Z80Reg::BC | Z80Reg::HL | Z80Reg::DE |Z80Reg::DE_Indirect | Z80Reg::HL_Indirect;
			}
			else if (z == 1) /* Second column. CPI, CPD, CPIR, CPDR*/
			{
				...
				regFlags = Z80Reg::A | Z80Reg::BC | Z80Reg::HL | Z80Reg::HL_Indirect;
			}
			else if (z == 2) /* Third column. INI, IND, INIR, INDR */
			{
				...
				regFlags = Z80Reg::B | Z80Reg::C | Z80Reg::HL | Z80Reg::HL_Indirect;
			}
    ...
    else 
	{
        switch (z) 
		{
            ...
            case 5: /* RETI/RETN*/
			{
				const uint16_t stackVal = CPUIF->ReadWord(z80_sp(pCPU));
				if (y==0)
				{
					snprintf(g_TTZ80DescBuf, kTTZ80DescLen, "PC is popped off the stack. Next instruction executed will be %s. "
						"IFF2 is copied back to IFF1 to restore the state of the "
						"interrupt flag before the non-maskable interrupt.", NumStr(stackVal));
				...
				else
				{
					snprintf(g_TTZ80DescBuf, kTTZ80DescLen, "PC is popped off the stack. "
						"Next instruction executed will be %s.", NumStr(stackVal));
					snprintf(g_TTZ80TitleBuf, kTTZ80TitleLen, "Return from interrupt");
				}
		 		regFlags = Z80Reg::SP_Indirect;
				break;
			}
            case 6: /* IM 0/1/2*/
			{
            ...
            case 7: /* LD I,A, LD R,A, LD A,I, LD A,R, RRD, RLD, NOP (ED), NOP (ED)*/
				...
				if (y == 0 || y == 2)
					regFlags =  Z80Reg::A | Z80Reg::I;
				else
				if (y == 1 || y == 3)
					regFlags =  Z80Reg::A | Z80Reg::R;
				else
					regFlags = Z80Reg::A | Z80Reg::HL_Indirect;
				break;
        }
    }
}
...
void GetToolTipInfoFromOpcode(uint16_t pc, ICPUInterface* CPUIF, FToolTipInstructionInfo& inst) 
{
	...
	{
        /* 8-bit load block */
		Do8BitLoadBlock(y, z, prefix, r, regFlags);
    }
    else if (x == 2) 
	{
		/* 8-bit ALU block */
            ...
            case 0:
                switch (y) 
				{
                    ...
                    case 1: /* EX AF,AF'*/
						...
						regFlags = Z80Reg::AF | Z80Reg::AF_ALT;
						break;
                    case 2: /* DJNZ*/
					{
            ...
            case 2: 
                {
                    switch (y) 
					{
                        case 0: /* LD (BC),A */
							...
							regFlags = Z80Reg::A | Z80Reg::BC_Indirect;
							break;
                        case 1: /* LD A,(BC) */
                        ...
                        case 2: /* LD (DE),A*/
							...
							regFlags = Z80Reg::A | Z80Reg::DE_Indirect;
							break;
                        case 3: /* LD A,(DE)*/
            ...
            case 6: /* LD s, n. Where s is (HL)/(IX+d)/(IY+d) or r*/
				if (y == 6) 
				{
					if (prefix) // LD (IX+d)/(IY+d), n
						snprintf(g_TTZ80DescBuf, kTTZ80DescLen, "Load indexed memory location %s with immediate data %s.", GetRegName(r[y]).c_str(), NumStr(CPUIF->ReadByte(pc+1)));
					else // LD (HL), n
    ...
    else 
	{
        switch (z) 
		{
            case 0: /* RET NZ/Z/NC/C/PO/PE/P/M*/
				...
				regFlags = Z80Reg::F | Z80Reg::SP_Indirect;
				break;
            case 1:
                if (q == 0) /* POP qq (register pair)*/
				{
					...
					regFlags = rp2[p] | Z80Reg::SP_Indirect;
				}
                else 
				{
                    switch (p) 
            ...
            case 3:
                switch (y) 
				{
                    ...
                    case 7: /* EI*/
						...
						break;
                    case 1: /* CB prefix */
                        DoCBPrefix(prefix, r, pc, CPUIF, regFlags);
						break;
                }
                break;
                ...
                else 
				{
                    switch (p) 
					{
                        ...
                        case 3: /* DBL PREFIX*/
							...
							break;
                        case 2: /* ED prefix */
							DoEDPrefix(prefix, r, rp, pc, CPUIF, regFlags);
							break;
                    }
                }
                break;
			...
			{
				const char* pchn = NumStr(CPUIF->ReadByte(pc));
				switch (y)
				{
					...
					case 3: // SBC A, n
						...
						break;
					case 4: // AND n
						snprintf(g_TTZ80DescBuf, kTTZ80DescLen, "Logical AND A with immediate data %s. Result stored in A.", pchn);
						break;
					case 5: // XOR n
						snprintf(g_TTZ80DescBuf, kTTZ80DescLen, "Exclusive OR A with immediate data %s. Result stored in A.", pchn);
						break;
					case 6: // OR n
						snprintf(g_TTZ80DescBuf, kTTZ80DescLen, "Logical OR A with immediate data %s. Results stored in A", pchn);
						break;
					case 7:
					{
						snprintf(g_TTZ80DescBuf, kTTZ80DescLen, "Subtract %s from A but discard result.\n\n"
...
void ShowCodeToolTipZ80(FCodeAnalysisState& state, const FCodeInfo* pCodeInfo)
{
	...
	std::string regStr = GenerateRegisterValueString(instrInfo.RegFlags, state.CPUInterface);
	...
	{
		ImGui::Text(instrInfo.Description.c_str());
	}
