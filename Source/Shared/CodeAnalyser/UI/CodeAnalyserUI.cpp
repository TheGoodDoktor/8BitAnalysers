...
// sam. split this out so we can call it from tooltips with other UI content.
void DrawSnippetContent(FCodeAnalysisState& state, FCodeAnalysisViewState& viewState, const FAddressRef addr, int noLines /* = 10 */)
{
	const FCodeAnalysisBank* pBank = state.GetBank(addr.GetBankId());
	if (pBank != nullptr)
	{
...
{
	ImGui::BeginTooltip();
	DrawSnippetContent(state, viewState, addr, noLines);
	ImGui::EndTooltip();
}
...
EDataItemDisplayType GetDisplayTypeForBitmapFormat(EBitmapFormat bitmapFormat)
{
	...
	case EBitmapFormat::Sprite3Bpp_PCE:
		return EDataItemDisplayType::Sprite3Bpp_PCE;
...
EBitmapFormat GetBitmapFormatForDisplayType(EDataItemDisplayType displayType)
{
	...
	case  EDataItemDisplayType::Sprite3Bpp_PCE:
		return EBitmapFormat::Sprite3Bpp_PCE;
